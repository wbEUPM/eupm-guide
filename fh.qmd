...
Step 1 Compute the selected direct area estimators ¥, and estimators of their corresponding sampling variances (perhaps previously smoothed)
...
With eblupFH, the model does not run if there is perfect multicollinearity between variables, so we need to omit them in advance.
