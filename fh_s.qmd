...
comb_Data_poor_in <- comb_Data_poor[complete.cases(comb_Data_poor$poor),]
var_smooth_model <- lm(log(vardir) ~ n + poor, data = comb_Data_poor_in)
summary(var_smooth_model)

residual_var <-  summary(var_smooth_model)$sigma^2
comb_Data_poor_in$var_smooth <- exp(predict(var_smooth_model)) * exp(residual_var / 2)
comb_Data_poor_in$se_smooth <- sqrt(comb_Data_poor_in$var_smooth)
comb_Data_poor_in$cv_smooth <- comb_Data_poor_in$se_smooth/ comb_Data_poor_in$poor
