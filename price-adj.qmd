...
To demonstrate how the proposed deflators are estimated in R, we use the 2019 Poland Household Budget Survey (HBS). In this analysis, we aim to estimate spatial price deflators at the NUTS 2 level (regions, *województwa*), which is the lowest NUTS level available in the dataset.
...
###	Six Steps to Calculalate Full-coverage Deflator

You can follow the following steps to get the full-coverage deflator. 

#### Step 1: Calculate the **household-level** food Paasche index $P_h^{food}$ in @eq-full-coverage. The formula is already defined in @eq-fpaasche. {.unnumbered}
...
#### Step 2: Compute the Paasche spatial food price index at NUTS 2 (**group $g$** as in ${SPI}_g^{food}$ in @eq-ph-nfood). Note that this is different from the household-level food Paasche in @eq-fpaasche. {.unnumbered} 
...
#### Step 3: Compute $w_z$, the average food budget share of the bottom deciles of the distribution of per capita expenditure {.unnumbered}
...
#### Step 4: Compute the group level poverty lines using the CBN method in order to derive the implicit deflator $\widetilde{SPI_g} =  \frac{z_g}{E[z_g]}$ in @eq-ph-nfood  {.unnumbered}
...
#### Step 5: Compute the $P_h(g)^{nfood}$ in @eq-full-coverage. {.unnumbered}
...
#### Step 6. Compute the full-coverage index. {.unnumbered} 
...
### Saving and displaying table results with tinytable (optional) ---- 
```{r}
