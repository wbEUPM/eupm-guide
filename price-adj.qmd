...
This chapter covers two approaches to estimate spatial deflators. The first approach is to use the Paasche price index. In this approach, we start calculating the food Paasche and the rent to extend the coverage of the price index without the price data in non-food expenditure.

To illustrate how the food Paasche can be implemented using R, we use Poland 2019 HBS. Suppose we want to estimate the price deflator at the NUTS 2 (Regions, *regioy*) level in Poland.
...
In addition to the data prepared from the HBS, a calorie conversion table is needed to convert food quantities consumed into calories. In this analysis, the author primarily used publicly available sources such as https://www.nutritionix.com/, though additional sources were also consulted. Given the country-specific nature of food consumption, input from local experts would be valuable in finalizing the calorie conversion table.
...
In this section, we calculate two versions of the food Paasche index. The first is the food-only index, and the second is the food plus rent price index, which will be further explained in Section 3.2.2 (to be update).
