...
The calculation of food + rent Paasche index is conducted in section xxx (tbu). 
...
After getting the consumption aggregate data from the HBS data, the following processed data are used to estimate the price deflators. All data sets are saved as .RDS. Each of these data are explained below.
...
```
Table: {#tbl-idata} Processed HBS data to be used in this chapter
```{r, echo = FALSE}
kable(idata)
```
...
### Getting the elements to get Paasche Indexes 
...
### Calorie Table Preparation 
In addition to the listed data sets in @tbl-idata, a calorie conversion table is needed to convert food quantities consumed into calories. In this analysis, the authors primarily used publicly available sources such as https://www.nutritionix.com/, though additional sources were also consulted. Given the country-specific nature of food consumption, input from local experts would be valuable in finalizing the calorie conversion table.

The final conversion table in Excel is saved in folder data_in. 
...
### Preparation: Food calorie conversion table ----
```{r}
...
###	Regional (Nuts2) Poverty Lines Estimation
#### CBN poverty lines: FPL
#### CBN poverty lines: FPL
#### CBN poverty lines: non-food allowances
...
###	Calculation of FULL COVERAGE Deflator
```{r}
