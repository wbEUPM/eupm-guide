...
#### Spatial Plot of Changes

```{r}

shp_dt <- st_as_sf(shp_dt, crs = 4326, agr = "constant")

compute_slope <- function(mcpe_obj) {
  y_df <- mcpe_obj$eblup
  time_period <- 0:(ncol(y_df) - 1)  # For 2012, 2013, 2014

  # Apply row-wise regression to get slope
  slopes <- apply(y_df, 1, function(y) {
    lm(y ~ time_period)$coefficients[2]
  })

  return(slopes)
}
shp_dt$slope <- compute_slope(mcpe_obj = mcpemfh2_obj)



### Looking at the slope of the gradient

shp_dt |>
  ggplot() + 
  geom_sf(aes(geometry = geometry, fill = slope), color = "white", linewidth = 0.2) +
  scale_fill_gradient2(
    low = "blue", mid = "white", high = "red",
    midpoint = 0,
    name = "Slope of Poverty Rate"
  ) +
  theme_minimal() +
  labs(
    title = "Gradient of Poverty Change (2012–2014)",
    subtitle = "Positive = Increasing poverty, Negative = Decreasing poverty"
  )
