...
stfh_obj <- pbmseSTFH(Y ~ X1 + X2, D, T, vardir = Var, spacetimeprox, data = spacetime)
...
stfh_cv <- 100 * sqrt(STFH$mse) / STFH$est$eblup 

direct_cv <- 100 * sqrt(spacetime$Var) / spacetime$Y

results <- data.frame(area = spacetime$Area, 
                      time = spacetime$Time, 
                      direct = spacetime$Y, 
                      eblup.STFH = STFH$est$eblup, 
                      cv.DIR, 
                      cv.STFH)
