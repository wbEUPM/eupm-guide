...
### ok lets take a portion of the income data and take random samples of 10 areas 
### to see if models converge from estimation
...
compute_stfh <- function(area_list){
  
  ### the cut relevant data
  shp_dt <- 
    shp_dt %>%
    filter(prov %in% area_list)
...
#### pick a random 10 provinces to estimate models from

draw_samples <- function(seed, ...){
...
stfh_obj <- lapply(X = sample_set,
                   FUN = function(x){
                     
                     stfh_obj <- tryCatch({
                       
                       compute_stfh(area_list = x)
                       
                     }, error = function(e){
                       
                       message(sprintf("Error developing STFH model", e$message))
...
### lets check the correlation between time period of the original direct estimates
compute_correlation <- function(area_list){
  
  cor_dt <- cor(prov_dt[prov %in% area_list, c("y2012", "y2013", "y2014")])  
  
  x <- mean(cor_dt[lower.tri(cor_dt)])
  
  return(x)
  
}

coreval_dt <- lapply(X = sample_set,
                     FUN = compute_correlation)

coreval_dt <- as.data.table(Reduce("rbind", coreval_dt))

conv_list <- 
  lapply(X = stfh_obj,
         FUN = function(x){
           
           status <- x$est$fit$convergence
           
           return(status)
           
         })

conv_list <- as.data.table(Reduce("rbind", conv_list))

coreval_dt <- coreval_dt %>%
  setnames("V1", "cor") %>%
  cbind(conv_list %>% 
          setnames("V1", "convstatus"))
